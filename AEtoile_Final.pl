...
test_AS:-
	...
	affiche_solution(Fin,Qu3).


%A etoile
aetoile(A,B,_):- empty(A), empty(B), print("PAS de SOLUTION : L’ETAT FINAL N’EST PAS ATTEIGNABLE !").

aetoile(Pf,Pu,Qs):-suppress_min([_,U], Pf, _),final_state(U), affiche_solution(U,Qs).

aetoile(Pf, Pu, Qs) :-
    suppress_min([Val,U], Pf, Pf1), 
    not(final_state(U)),
    suppress([U,Val,Pere,A], Pu, Pu1),
    insert([U,Val,Pere,A], Qs, Q1),
    findall([S, NewVal, U,A], (expand(U,Val,S,NewVal,A)), ListeElement),
    loop_successors(ListeElement, Pu1, Pf1, Q1, Pu3, Pf3),
    aetoile(Pf3,Pu3,Q1).
...
%Test expand
test_expand(S,Fs,Hs,Gs):-
    initial_state(Ini),
    expand(Ini, [_,_,0], S, [Fs,Hs,Gs], right).

%Loop successors
loop_successors([[S,[F,H,G],Pere,A]|Rest], Pu, Pf, Q, Pu3, Pf3):-
    (   belongs([S, _, _, _] ,Q) -> %Si S est connu dans Q
    	loop_successors(Rest, Pu,Pf,Q, Pu3, Pf3)  %On ne fait rien
    ;
    	(belongs([S, [OldFs,OldHs,OldGs], OldPere, OldA], Pu) ->  %Sinon, s'il est connu dans Pu on garde la meilleure évaluation
        	( F < OldFs ->  %Si la nouvelle évaluation est meilleure que l'ancienne on remplace les anciens termes par les nouveaux
				suppress([S, [OldFs,OldHs,OldGs], OldPere, OldA],Pu,Pu2),
                suppress([[OldFs,OldHs,OldGs], S],Pf,Pf2),
                insert([S, [F,H,G], Pere, A], Pu2, Pu3),
        ...
        ;%Sinon, S est une nouvelle situation il faut donc l'ajouter
        	insert([S, [F,H,G], Pere, A], Pu, Pu3),
            insert([[F,H,G], S], Pf, Pf3)
